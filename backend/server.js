...
app.get('/api/questions', async (req, res) => {
    try {
        ...
        for (let q of questions) {
            const [options] = await db.query('SELECT id, teks, tipe_most, tipe_least FROM options WHERE question_id = ?', [q.id]);
        ...
        res.status(500).json({ success: false, message: 'Gagal memuat soal dari database.' });
    }
});
...
app.post('/api/submit-test', async (req, res) => {
    ...
    try {
        ...
        await db.query(
            ...
            [
                attempt_id,
                scoreMost.D, scoreMost.I, scoreMost.S, scoreMost.C, scoreMost.star,
                scoreLeast.D, scoreLeast.I, scoreLeast.S, scoreLeast.C, scoreLeast.star,
                scoreChange.D, scoreChange.I, scoreChange.S, scoreChange.C, scoreChange.star
            ]
        );
        ...
        console.error(error);
        res.status(500).json({ success: false, message: 'Terjadi kesalahan pada server saat menghitung skor.' });
    }
});
...
app.post('/api/register', async (req, res) => {
    const { nama_lengkap, umur, pendidikan_terakhir, pekerjaan, jenis_kelamin } = req.body;
    ...
    try {
        ...
        res.status(500).json({ success: false, message: 'Gagal menyimpan data diri ke database.' });
    }
});
